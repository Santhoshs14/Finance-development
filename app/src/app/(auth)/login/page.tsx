...
export default function LoginPage() {
  const { signIn, signInWithGoogle, signInWithPasskey, user, loading } = useAuth();
  ...
  const handlePasskey = async () => {
    ...
    try {
      ...
      // A user cancelling the native prompt is not an error worth showing.
    ...
    <div className="flex min-h-screen items-center justify-center px-4 bg-background">
      <div className="w-full max-w-sm space-y-6">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Sign in to your financial OS</p>
        </div>
          ...
          <CardContent className="p-6 space-y-4">
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end">
                <Link href="/forgot-password" className="text-xs text-brand hover:text-brand/80">Forgot password?</Link>
              </div>
            ...
            {passkeySupported && (
              <button
                ...
                className="flex w-full items-center justify-center gap-3 rounded-lg border border-border bg-card py-2.5 text-sm font-medium text-foreground hover:bg-muted/50 transition-colors disabled:opacity-60"
              >
                {passkeyBusy ? (
        ...
        <p className="text-center text-sm text-muted-foreground">
          ...
          <Link href="/register" className="font-medium text-brand hover:text-brand/80">Sign up</Link>
        </p>
      </div>
    </div>
  );
