...
// limits across serverless invocations. Checked lazily on first use rather than
// at import time so `next build` (which sets NODE_ENV=production while merely
// collecting page data) is not blocked. Opt out with RATE_LIMIT_REQUIRE_KV=false.
let kvGuardChecked = false;

function assertKvConfigured() {
  if (kvGuardChecked) return;
  kvGuardChecked = true;
  if (process.env.NODE_ENV !== "production" || HAS_KV) return;
  ...
  if (process.env.RATE_LIMIT_REQUIRE_KV !== "false") {
    throw new Error(msg);
  }
  console.warn(msg);
}
